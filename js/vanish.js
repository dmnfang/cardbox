...
function initVanish() {
  vRound = 0;
  vActiveTeam = 0;
  const cards = S.activeCards;
  const [cols, rows] = autoVanishGrid(cards.length);
  const total = cols * rows;
  const unique = [...new Map(cards.map(c => [c.word.toLowerCase(), c])).values()];
...
function renderVanishGrid(showGhosts) {
  const [cols, rows] = autoVanishGrid(vPoolCards.length);
  ...
  vGridCards.forEach((card, i) => {
    ...
    if (ghost) {
      ...
      icon.innerHTML = GHOST_ICON_HTML;
...
function vanishAction() {
  ...
  else if (S.teamCount === 0) openGuessModal();
}
...
function doShuffle() {
  ...
  cells.forEach((cell, i) => {
    ...
    cell.innerHTML = GHOST_SVG_WHITE;
...
// POINT TICKER
// ══════════════════════════════════════════════
function startPointTicker(reset = false) {
  if (reset) vCurrentPoints = 100;
  updatePointsDisplay();
  clearInterval(vPointTicker);
  vPointTicker = setInterval(() => {
    if (vCurrentPoints > 5) {
      vCurrentPoints -= 5;
      updatePointsDisplay();
    }
...
function updatePointsDisplay() {
  const btn = document.getElementById('vanish-action-btn');
  if (btn && S.teamCount > 0) {
    btn.textContent = `${vCurrentPoints} pts`;
  }
}
...
function submitGuess() {
  ...
  const matchIdx = remaining.find(i =>
    vGridCards[i].word.trim().toLowerCase() === guess.trim().toLowerCase()
  );
